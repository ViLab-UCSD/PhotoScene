...
def getBsdfIdDict(xmlFile, modelStrIdDict):
...
def selectViewAndSaveMask(cfg, candidateCamIdList, bsdfIdDict, imWidth, imHeight, labelMapDict, \
    ...
    if isOverwrite or not osp.exists(selectedViewFullDictFile):
        ...
        for mat in selectedViewDict.keys():
            ...
            maskWarpedJointOut = th.zeros_like(orInsSelect[0], requires_grad=False) # [1, 1, 240, 320] -> with masked out un-aligned normal pixels
            maskWarpedOut = th.zeros_like(orInsSelect[0], requires_grad=False) # [1, 1, 240, 320] -> also consider un-aligned normal pixels
            ...
            for idx, orIns in enumerate(orInsSelect):
                ...
                # # <<<< after finding matching, optimize for better centers and lengths
                warpedMatPart = transformInp(orMatPart.float()*orIns.float(), center1, centerOpt, length1, lengthOpt) * snSemMask
            ...
            for idx, orIns in enumerate(orInsSelect):
                ...
                #optimizer = th.optim.Adam([centerOpt.requires_grad_()], lr=1e-4)
                orIns = orIns.float()
                _, _, H, W = orIns.shape
                xTH = 1 / W * 2
                yTH = 1 / H * 2
                ...
                # # <<<< after finding matching, optimize for better centers and lengths
                warpedSnMask = transformInp(snMask.float()*snSemMask.float(), centerOpt, center1, lengthOpt, length1) * orIns
                ...
                targetAlbedoOut    = warpedSnMask      * warpedAlbedo      + (1-warpedSnMask)      * targetAlbedoOut
                targetRoughOut     = warpedSnMask      * warpedRough       + (1-warpedSnMask)      * targetRoughOut
                targetNormalOut    = warpedSnMask      * warpedSnNormal    + (1-warpedSnMask)      * targetNormalOut
...
def updateXMLFromTotal3D(cfg):
    def saveLayoutMesh(objInfoDict, matInfoDict, layoutArr, meshSaveDir, blenderApplyUvCmdPre, shapeCntInit=0, bsdfCntInit=0):
        def vToLine(v):
            return 'v %f %f %f\n' % (v[0], v[1], v[2])

        def newObjInfo(shapeName, objPath, matList):
            objInfo = {'shape_id': shapeName, 'objPath': objPath, 'matList': matList, 'scale': np.array([1.0, 1.0, 1.0]), \
                    'isEmitter': False, 'emitterVal': None, \
    ...
    def saveObjectMesh(objInfoDict, matInfoDict, objDictList, total3dOutputDir, meshSaveDir, blenderApplyUvCmdPre, shapeCntInit=0, bsdfCntInit=0):
        ...
        for objIdx, objDict in enumerate(objDictList):
            ...
            objInfoDict[shapeCnt] = newObjInfo('%d_%d_object' % (classId[0], objIdx), objUV, [bsdfName], opList, attribList)
            shapeCnt += 1
    ...
    shapeCnt, bsdfCnt = saveLayoutMesh(objInfoDict, matInfoDict, \
        layoutArr, cfg.meshSrcSaveDir, cfg.blenderApplyUvCmdPre)
    shapeCnt, bsdfCnt = saveObjectMesh(objInfoDict, matInfoDict, \
        objDictList, cfg.total3dOutputDir, cfg.meshSrcSaveDir, cfg.blenderApplyUvCmdPre, shapeCntInit=shapeCnt, bsdfCntInit=bsdfCnt)
...
def saveCamFile(newCamFile, newCamIdFile, newCamDir, camIdList, camPoseDictList):
    ...
    print('New cam file for all ScanNet views is saved at %s' % newCamFile)
    print('New camId file for all ScanNet views is saved at %s' % newCamIdFile)
...
def renderOpenRoom(cfg):
    ...
    def adjustFileNameByCamId(cfg, item):
        ...
        # print('---> Adjusting view ID for %s ...' % item)
        if renderDict[item]['tag']:
            renderDir = osp.join(cfg.preprocessDir, item)
            tmpDir = renderDir.replace(item, '%s_tmp' % item)
            os.system('mkdir -p %s' % tmpDir)
            for j in range(cNum):
                i = cNum - j - 1
                old = osp.join(renderDir, '%s_%d.%s' % (renderDict[item]['fn'], i+1, renderDict[item]['ext'])  )
                assert(osp.exists(old))
                new = osp.join(tmpDir   , '%s_%s.%s' % (renderDict[item]['fn'], cfg.camIdList[i], renderDict[item]['ext']) )
                cmd = 'mv %s %s' % (old, new )
                os.system( cmd)
            os.system('rm -r %s' % renderDir)
            assert(osp.exists(tmpDir))
            os.system('mv %s %s' % (tmpDir, renderDir))

    # Render 
    if cfg.renderPartId: # Render Part Label
