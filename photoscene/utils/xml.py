...
from utils.io import readViewDict, loadInvRenderData, getMaterialSavePathDict, getObjMatVertFaceDict, \
    saveObjFromMatVertFaceDict, checkVtExist, modifyMaterialName, readGraphDict
import copy
...
def genBaselineMatAndPathDict(cfg, baseline, tag=None, refW=320, refH=240):
    ...
    if baseline == 'median' or baseline == 'invrender' or baseline == 'white':
        for mat, vId in selectedViewDict.items():
            ...
            mask_ref = th.from_numpy(np.asarray(
                Image.open(maskSNPath).resize((refW, refH))).astype(np.float) / 255).unsqueeze(0).unsqueeze(1)
        ...
        m = 'init' if baseline == 'first' else 'final' if baseline == 'second' else None
        baselineMatDict = getMaterialSavePathDict(cfg, m)
...
def saveNewBaselineXML(cfg, baseline):
    ...
    outPath = cfg.xmlMedianFile if baseline == 'median' else cfg.xmlInvRenderFile if baseline == 'invrender' else None
...
def getOptUvDict(cfg, xmlFile):
    # Apply New UV paramters from first optimization and warping offset
    # Return a objMatUVDict for objects in xmlFile
    #      {objID: {mat: uvDict}}

    selectedGraphDict = readGraphDict(cfg.selectedGraphFile)
    selectedViewDict = readViewDict(cfg.selectedViewFile)
    ...
    for objID in sorted(objInfoDict.keys()):
        ...
        for mat in matList:
            ...
            if mat in selectedGraphDict.keys():
                warpInfoFile = cfg.warpInfoByNameId % (mat, selectedViewDict[mat])
                ...
                uvDictPath = cfg.uvDictPathByName % mat
