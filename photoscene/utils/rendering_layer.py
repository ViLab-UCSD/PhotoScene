...
class MicrofacetUV:
    ...
    def getRot(self, a, b): # get rotation matrix from vec a to b
        ...
        R = th.eye(3).view(1, 1, 3, 3).repeat(H, W, 1, 1).to(self.device) + nu_skew + th.matmul(nu_skew, nu_skew) / (1 + c)
    ...
    def fromUV(self, valMap, uvDict=None):
        if uvDict is None:
            ...
            assert(uvDict['rot'] is not None and uvDict['trans'] is not None and uvDict['scaleBase'] is not None and uvDict['logscale'] is not None)
        ...
        return th.nn.functional.grid_sample(valMap, grid.expand(valMap.shape[0], -1, -1, -1), mode='bilinear', padding_mode='border', align_corners=False)
    ...
    def applyMaskLoss(self, aMap, bMap):
        ...
        aB, aC = aMap.shape[0], aMap.shape[1]
        bB, bC = bMap.shape[0], bMap.shape[1]
        weightB, weightC = self.lossWeightMap.shape[0], self.lossWeightMap.shape[1]
    ...
    def eval(self, albedoMap, normalMap, roughMap, uvDict=None, returnBRDF=False, eps=1e-10):
        ...
        normalPred = th.matmul(self.Rot, normalInit.permute(0, 2, 3, 1).unsqueeze(-1)).squeeze(-1).permute(0, 3, 1, 2).to(self.device)
        ...
        img = f * n_dot_l * self.envmap * self.envWeight.expand([1, self.envWidth * self.envHeight, 3, self.imHeight, self.imWidth] )
